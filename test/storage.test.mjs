...
import { getSettings, saveSettings, getClosedTabs, addClosedTab, clearClosedTabs, removeClosedTab } from '../src/utils/storage.js';
...
describe('Storage Utils', () => {
    ...
    describe('getSettings', () => {
        ...
        it('should return saved settings', async () => {
            chromeMock.storage.sync.get.yields({ timeout: 60, unit: "hours", historyLimit: 200 });
            const settings = await getSettings();
    ...
    describe('saveSettings', () => {
        it('should save settings object', async () => {
            ...
            expect(chromeMock.storage.sync.set.calledWith({ timeout: 45, unit: "days", historyLimit: 50 })).to.be.true;
        });
    });
    ...
    describe('addClosedTab', () => {
        it('should add a tab to history and generate ID', async () => {
            ...
            chromeMock.storage.sync.get.yields({});
            chromeMock.storage.local.get.yields({ closedTabs: [] });
            chromeMock.storage.local.set.yields();

            const tab = { title: 'Test', url: 'http://test.com', closedAt: 12345 };
            await addClosedTab(tab);

            const savedTab = chromeMock.storage.local.set.firstCall.args[0].closedTabs[0];
        ...
        it('should limit history to configured limit', async () => {
            ...
            chromeMock.storage.local.get.yields({ closedTabs: existing });
            chromeMock.storage.local.set.yields();
            ...
            expect(args.closedTabs.length).to.equal(10);
            expect(args.closedTabs[0].title).to.equal(tab.title);
        });
        ...
        it('should allow infinite history if limit is -1', async () => {
            ...
            const args = chromeMock.storage.local.set.firstCall.args[0];
            expect(args.closedTabs.length).to.equal(151);
        });
    });
    ...
    describe('removeClosedTab', () => {
        it('should remove a tab by ID', async () => {
            ...
            const savedTabs = chromeMock.storage.local.set.firstCall.args[0].closedTabs;
    ...
    describe('clearClosedTabs', () => {
        it('should clear history', async () => {
            ...
            expect(chromeMock.storage.local.set.calledWith({ closedTabs: [] })).to.be.true;
        });
    });
});
