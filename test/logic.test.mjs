...
describe('Background Logic', () => {
    ...
    describe('checkTabs', () => {
        it('should close expired tabs', async () => {
            ...
            chromeMock.storage.sync.get.callsFake((keys, callback) => {
                if (typeof callback === 'function') callback({ timeout: 30, unit: 'minutes' });
            });
            ...
            // Mock tabs
            const tabs = [
                { id: 1, active: false, pinned: false, audible: false, title: 'Expired', url: 'http://expired.com' },
                { id: 2, active: true, pinned: false, audible: false, title: 'Active', url: 'http://active.com' }
            ];
    ...
    describe('updateBadge', () => {
        it('should show lock icon for protected tab', async () => {
            ...
            expect(chromeMock.action.setBadgeText.calledWith({ tabId: 123, text: "🔒" })).to.be.true;
        });
